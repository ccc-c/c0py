...
echo ""
echo "Running QEMU test (15 seconds)..."

# Run QEMU in background and redirect properly
# xv6 uses -bios none so QEMU doesn't load OpenSBI
# All 4 harts will start from our _start at 0x80000000
qemu-system-riscv64 -nographic -smp 4 -machine virt -bios none -kernel os.elf > /tmp/test_out.txt 2>&1 &
QEMU_PID=$!
sleep 15
