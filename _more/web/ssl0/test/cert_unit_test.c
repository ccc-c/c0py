...
void test_base64_decode_simple() {
    ...
    const char *input = "SGVsbG8=";  // "Hello" in base64
    uint8_t output[16];
    size_t out_len;
    
    int ret = base64_decode(input, strlen(input), output, &out_len);
    assert(ret == 0);
    assert(out_len == 5);
...
void test_base64_decode_multiple_blocks() {
    ...
    // Simple test: 4 chars = 3 bytes
    const char *input = "QUJD";  // "ABC"
    uint8_t output[16];
    size_t out_len;
...
void test_base64_decode_with_padding() {
    ...
    // Simple test without complex padding
    const char *input = "AAE=";  // 3 chars + padding
    uint8_t output[16];
    size_t out_len;
...
void test_base64_decode_empty() {
    ...
    int ret = base64_decode(input, 0, output, &out_len);
    assert(ret == 0);
    assert(out_len == 0);
...
void test_get_asn1_length_short_form() {
    ...
    uint8_t data[] = {0x30, 0x10};  // SEQUENCE, length 16
    size_t len, header_len;
    
    int ret = get_asn1_length(data, 2, &len, &header_len);
    assert(ret == 0);
    assert(len == 16);
    assert(header_len == 2);
...
void test_get_asn1_length_long_form() {
    ...
    uint8_t data[] = {0x30, 0x82, 0x01, 0x00};  // SEQUENCE, length 256
    size_t len, header_len;
    
    int ret = get_asn1_length(data, 4, &len, &header_len);
    assert(ret == 0);
    assert(len == 256);
    assert(header_len == 4);
...
void test_get_asn1_length_very_long_form() {
    ...
    uint8_t data[] = {0x30, 0x83, 0x01, 0x00, 0x00};  // SEQUENCE, length 65536
    size_t len, header_len;
    
    int ret = get_asn1_length(data, 5, &len, &header_len);
    assert(ret == 0);
    assert(len == 65536);
    assert(header_len == 5);
...
void test_get_asn1_length_invalid() {
    ...
    int ret = get_asn1_length(data, 1, &len, &header_len);
    assert(ret == -1);
...
void test_extract_integer_simple() {
    ...
    uint8_t data[] = {0x02, 0x03, 0x01, 0x02, 0x03};  // INTEGER, len=3, value=0x010203
    uint8_t output[16];
    size_t out_len;
    
    int ret = extract_integer(data, 5, output, &out_len);
    assert(ret == 0);
    assert(out_len == 3);
...
void test_extract_integer_with_leading_zero() {
    ...
    uint8_t data[] = {0x02, 0x04, 0x00, 0x01, 0x02, 0x03};  // INTEGER, len=4, value with leading zero
    uint8_t output[16];
    size_t out_len;
    
    int ret = extract_integer(data, 6, output, &out_len);
    assert(ret == 0);
    assert(out_len == 3);  // leading zero removed
    assert(memcmp(output, "\x01\x02\x03", 3) == 0);
...
void test_extract_integer_not_integer() {
    ...
    uint8_t data[] = {0x30, 0x03, 0x01, 0x02, 0x03};  // Not INTEGER
    uint8_t output[16];
    size_t out_len;
...
void test_x509_parse_from_pem() {
    printf("test_x509_parse_from_pem...\n");
