...
char *instr_name = "LLA ,IMM ,JMP ,JSR ,BZ  ,BNZ ,ENT ,ADJ ,LEV ,LI  ,LC  ,SI  ,SC  ,PSH ,OR  ,XOR ,AND ,EQ  ,NE  ,LT  ,GT  ,LE  ,GE  ,SHL ,SHR ,ADD ,SUB ,MUL ,DIV ,MOD ,LF  ,SF  ,IMMF,ITF ,ITFS,FTI ,FADD,FSUB,FMUL,FDIV,FEQ ,FNE ,FLT ,FGT ,FLE ,FGE ,PRTF,OPEN,READ,CLOS,PRTF,MALC,FREE,MSET,MCMP,EXIT,FOPN,FWRT,FCLS,";
long debug = 0;
...
long run(long *pc, long *bp, long *sp, char *d_base, long *t_base, long poolsz) {
  ...
  while (1) {
    ...
    else if (i == FWRT) a = (long)fwrite((void *)to_addr(sp[3], (long)d_base, poolsz), sp[2], sp[1], (void *)*sp);
    else if (i == FCLS) a = (long)fclose((void *)*sp);
...
int main(int argc, char **argv) {
    ...
    // 取得離開指令的 Offset。在 c4.c 編譯時，我們在 text 結尾加了 PSH 和 EXIT
    // PSH (1 指令) + EXIT (1 指令) = 2 個 long。
