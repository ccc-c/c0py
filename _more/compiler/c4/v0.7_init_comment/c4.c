...
// 修改者: (新增全域/局部變數宣告時初始化、新增精準的視覺化錯誤提示 ^)
      ...
      if (tk == '(') {
        ...
        *++e = LEV;
        ...
        if (tk == Brak) {
          ...
          if (tk == Assign) {
            next();
            if (tk == Num) { *(long *)id->v_val = ival; next(); }
            else if (tk == NumF) { *(double *)id->v_val = fval; next(); }
            else if (tk == '"') { *(long *)id->v_val = ival; next(); while(tk == '"') next(); }
            else if (tk == '-') {
              next();
              if (tk == Num) { *(long *)id->v_val = -ival; next(); }
              else if (tk == NumF) { *(double *)id->v_val = -fval; next(); }
