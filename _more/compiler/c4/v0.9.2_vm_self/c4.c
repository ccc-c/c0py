// c4_elf_phase3.c - C in four functions (ELF Phase 3: Pure POSIX I/O)
//
...
void write_elf(char *filename) {
  ...
  // [修改] 徹底 POSIX 化，使用 open, write, close。
  // 1537 在 macOS 代表 O_WRONLY(1) | O_CREAT(512) | O_TRUNC(1024)
  // 420  代表八進位 0644 權限
...
long compile(long argc, char **argv)
{
  ...
  // [修改] 將 write 註冊為 syscall，移除 FILE。
