...
JitFunc jit_compile(Instruction *program, int len) {
    ...
    system("clang -shared -o jit_func.so jit_func.c -fPIC 2>/dev/null");
...
int main() {
    ...
    int result2 = jit();
    printf("Compiled C code: int jit_func() { a=1; b=2; a=a*b; b=3; a=a+b; return a; }\n");
    printf("Result: %d\n\n", result2);

    printf("========================================\n");
    printf("Interpretation: 每條指令都要透過 switch-case\n");
    printf("JIT: 在執行時即時編譯成機器碼，直接執行\n");
    printf("========================================\n");
    return 0;
}
